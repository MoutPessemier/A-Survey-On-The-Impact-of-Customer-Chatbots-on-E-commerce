...
From the 65 entries, we had to throw 5 away:
...
```{r}
others <- filter(d, chatbot == 'T-Mobile' | chatbot == 'Orange' | chatbot == 'KPN')
OFAQ <- filter(others, taskCategory == "FAQ")
OComplaintsQ <- filter(others, taskCategory == "Complaint")
OSalesQ <- filter(others, taskCategory == "Sales")
OPersonalQ <- filter(others, taskCategory == "Personalised Question")
```
...
```{r}
for(i in 1: 19){
  yes <- table(OFAQ[[paste0("q", i)]])["1"]
  no <- table(OFAQ[[paste0("q", i)]])["2"]
  ...
  b <- barplot(table(OFAQ[[paste0("q", i)]]),
          main=questions1[i],
          ylab="Count",
          names.arg = ynlabels,
          col="blue",
          ...
          cex.main = fsize)
  abline(v=c(1.3) , col="grey")
  text(b, y=ynpos, paste("n: ", ynval , sep=""), cex=0.7, col = "red")
}
```

#### Complaints

This is commented out because there simply are no complaints between the three providers.

```{r}
# for(i in 1: 19){
#   yes <- table(OComplaintsQ[[paste0("q", i)]])["1"]
#   no <- table(OComplaintsQ[[paste0("q", i)]])["2"]
#   
#   yes[is.na(yes)] <- 0
#   no[is.na(no)] <- 0
#   
...
#   }
#   
#   b <- barplot(table(OComplaintsQ[[paste0("q", i)]]),
#           main=questions1[i],
#           ylab="Count",
#           names.arg = ynlabels,
#           col="blue",
...
```{r}
for(i in 1: 19){
  yes <- table(OSalesQ[[paste0("q", i)]])["1"]
  no <- table(OSalesQ[[paste0("q", i)]])["2"]
  ...
  b <- barplot(table(OSalesQ[[paste0("q", i)]]),
          main=questions1[i],
          ylab="Count",
          names.arg = ynlabels,
          col="blue",
...
```{r}
for(i in 1: 19){
  yes <- table(OPersonalQ[[paste0("q", i)]])["1"]
  no <- table(OPersonalQ[[paste0("q", i)]])["2"]
  ...
  b <- barplot(table(OPersonalQ[[paste0("q", i)]]),
          main=questions1[i],
          ylab="Count",
          names.arg = ynlabels,
          col="blue",
...
### Putting the 3 categories in one graph
```{r}
for(i in 1: 19){
...
Satisfaction coefficient: $A+P/A+P+M+I$

Dissatisfaction coefficient: $P+M/A+P+M+I$
